...
with open('train_gpt_phase3.py', 'r', encoding='utf-8') as f:
    source = f.read()

# 1. Update XSA_LAST_N
source = re.sub(
    r'xsa_last_n = int\(os\.environ\.get\("XSA_LAST_N", 4\)\)',
    r'xsa_last_n = int(os.environ.get("XSA_LAST_N", 11))',
    source
)

# 2. Replace DistributedTokenLoader with CoprimeStride implementation
...
source = re.sub(
    r'class DistributedTokenLoader:.*?def load_state_dict.*?self\.stream\.load_state_dict\(stream_state\)',
    new_loader,
    source,
    flags=re.DOTALL
)
...
new_muon = """class TurboMuon(torch.optim.Optimizer):
...
with open('train_gpt_phase3_mod1.py', 'w', encoding='utf-8') as f:
    f.write(source)
