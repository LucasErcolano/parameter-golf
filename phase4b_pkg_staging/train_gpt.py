...
def eval_val_sliding_ttt(
    ...
    seq_len = args.train_seq_len
    docs = iter_document_segments(val_tokens, getattr(args, "bos_token_id", -1))
    ...
    for doc_idx, (doc_start, doc_end) in enumerate(docs):
        if eval_timeout_reached(args.eval_timeout_seconds, t0, device, collective=distributed):
            timed_out = True
            break
        doc_tokens = val_tokens[doc_start:doc_end].contiguous()
        if doc_tokens.numel() <= 1:
            continue
        if lora is not None and initial_lora_state is not None:
        ...
        for ci, windows in enumerate(chunk_windows):
            ...
            with torch.inference_mode():
                for bi in range(0, len(my_windows), batch_seqs):
                    if eval_timeout_reached(args.eval_timeout_seconds, t0, device):
                        timed_out = True
                        break
                    ...
                    if mixer is not None and mixer.tokens_seen >= mixer.min_tokens:
                        nll = mixer.score(logits, x_eval, y_eval, args.ttt_temperature)
                    else:
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str]):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            ...
            q_packed, q_numel = pack_int6_tensor(q)
            result[name + ".q"] = q_packed
            result[name + ".scale"] = s
            meta[name] = {"type": "int6_packed", "shape": list(t.shape), "numel": q_numel}
        else:
            q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        q_deq = q
        if isinstance(info, dict) and info.get("type") == "int6_packed":
            q_deq = unpack_int6_tensor(
                q,
                int(info["numel"]),
...
def main() -> None:
    ...
    log0(
        ...
        f"int6={n_params * 0.75 / 1e6:.2f}MB"
    )
    ...
    if args.eval_only:
        ...
        if master_process:
            ...
            log0(
                "eval:qdiag "
                f"raw={len(quant_raw)}B lzma={len(quant_blob)}B "
