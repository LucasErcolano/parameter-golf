...
def main() -> None:
    ...
    compiled_model = torch.compile(base_model, dynamic=False, fullgraph=True) if os.environ.get("TORCH_COMPILE", "1") == "1" else base_model
    ...
    while True:
        ...
        if args.grad_clip_norm > 0:
            gn = torch.nn.utils.clip_grad_norm_(base_model.parameters(), args.grad_clip_norm)
            if should_log_train:
                log0(f"step:{step} grad_norm:{gn.item():.4f}")
        for opt in optimizers:
            opt.step()
        zero_grad_all()
